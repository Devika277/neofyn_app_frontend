// lib/screens/account/change_tpin_screen.dart
...
class _ChangeTpinScreenState extends State<ChangeTpinScreen> {
  ...
  final _currentTpinController = TextEditingController();
  ...
  @override
  void dispose() {
    _currentTpinController.dispose();
  ...
  TextEditingController _getActiveController() {
    switch (_activeFieldIndex) {
      case 0:
        return _currentTpinController;
      case 1:
        return _newTpinController;
      case 2:
        return _confirmTpinController;
      default:
        return _currentTpinController;
    }
  }
  ...
  bool _allFieldsFilled() {
    return _currentTpinController.text.length == 6 &&
        _newTpinController.text.length == 6 &&
        _confirmTpinController.text.length == 6;
  }
  ...
  void _clearAll() {
    _currentTpinController.clear();
    _newTpinController.clear();
  ...
  Future<void> _changeTpin() async {
    final currentTpin = _currentTpinController.text.trim();
    final newTpin = _newTpinController.text.trim();
    final confirmTpin = _confirmTpinController.text.trim();

    if (currentTpin.length != 6) {
      setState(() => _errorMessage = 'Please enter current TPIN');
      return;
    }

    final validation = _validateTpin(newTpin);
    if (validation != null) {
    ...
    if (newTpin == currentTpin) {
      setState(() => _errorMessage = 'New TPIN must be different from current');
      return;
    }

    if (newTpin != confirmTpin) {
      setState(() => _errorMessage = 'New TPINs do not match');
      return;
    }
  ...
  Widget _buildKey(String number, double size, {bool isDelete = false, bool isEmpty = false}) {
    if (isEmpty) {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      // Current TPIN
                      _buildTpinField(
                        controller: _currentTpinController,
                        label: 'Current TPIN',
                      ...
                      if (_allFieldsFilled()) ...[
                        ...
                        GestureDetector(
                          onTap: _isLoading ? null : _changeTpin,
                          child: Container(
                            width: double.infinity,
                            height: 52,
                            decoration: BoxDecoration(
                      ...
                      Row(
                        ...
                        children: [
                          Icon(Icons.security, size: 14, color: Colors.white.withOpacity(0.3)),
